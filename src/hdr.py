...
class FakeHDR():
	...
	def process(self, image):
		...
		image = 1.0*image/255
		L = 1.0*S
		cv2.imwrite("original.png", L*255.0)
		I = self.wls(S)
		cv2.imwrite("illuminance.png", I*255.0)
		R = np.log(L+1e-22) - np.log(I+1e-22)
		cv2.imwrite("reflectance.png", R*255.0)
		R_ = self.srs(R, L)
		cv2.imwrite("reflectance_srs.png", R_*255.0)
		I_K = self.vig(L, 1.0 - L)
		k = 1
		for img in I_K:
			cv2.imwrite("vig"+str(k)+".png", img*255.0)
			k=k+1
